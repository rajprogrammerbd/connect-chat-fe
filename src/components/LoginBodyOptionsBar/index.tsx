        ...
        <div className="border-r-2 border-gray-100">
            <div className="mt-5 ml-2">
                ...
                <div className="profile-options">
                    {state.optionLists.map((list: OptionListsType) => (
                        <div
                            style={{
                                borderRight: (state.activeOptionId === list.id) ? "1px solid #1976d2" : "none"
                            }}
                            key={list.id}
                            className="option-items"
                        >
                            <list.icon key={list.id} size={list.size} color={(state.activeOptionId === list.id) ? "#1976d2" : list.color} />
